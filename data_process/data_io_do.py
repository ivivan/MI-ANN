...
def load_csvdata_Pearson(rawdata, time_steps,mode,scalardic):
    data = rawdata
    if not isinstance(data, pd.DataFrame):
        data = pd.DataFrame(data)
    ...
    all_train = np.stack((train_x,train_x_four,train_x_five), axis=-1)
    all_train = all_train.reshape(-1,time_steps*3)

    return dict(train=all_train,scalerone=scaler_do,scalertwo=scaler_ec,scalerthree=scaler_temp,scalerfour=scaler_ph,scalerfive=scaler_chlo), dict(trainyone=train_y,trainytwo=None,trainythree=None)
